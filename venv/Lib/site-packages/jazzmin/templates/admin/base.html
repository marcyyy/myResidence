...
<div class="wrapper">
    ...
    {% if not request.user.admin %}
            <div style=" align-content: center; justify-content: center; display: flex; margin-bottom: 0px;
                            color: #842029; background-color: #f8d7da; border-color: #f5c2c7; position: relative;
                                padding: 1rem 1rem; border: 1px solid transparent; border-radius: .25rem; font-weight: normal; " >
                        <a href="{% url 'admin:myResidence_admin_add' %}">Please fill out your profile form.</a>
            </div>
    {% endif %}
        ...
        <nav class="main-header navbar navbar-expand {{ jazzmin_ui.navbar_classes }}" id="jazzy-navbar">
            <ul class="navbar-nav">
                ...
                {% if jazzmin_settings.show_sidebar %}
                    <li class="nav-item">
                        <a class="nav-link" data-widget="pushmenu" href="#" role="button" style="color:#FAFAFA"><i class="fas fa-bars"></i></a>
                    </li>
                {% else %}
                    <li class="nav-item">
                ...
                {% for link in top_menu %}
                    <li class="nav-item d-none d-sm-inline-block{% if link.children %} dropdown{% endif %}">
                        ...
                        {% else %}
                            <a href="{{ link.url }}" class="nav-link" style="font-size:12px;color:rgba(250,250,250,0.9)" {% if link.new_window %}target="_blank" {% endif %}>{{ link.name }}</a>
                        {% endif %}
                    </li>
                {% endfor %}
            </ul>
            ...
            <ul class="navbar-nav ml-auto">
                ...
                {% if jazzmin_settings.language_chooser %}
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-left" id="jazzy-languagemenu" style="height: 500px; overflow-y: scroll;">
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link btn" data-toggle="dropdown" href="#" title="{{ request.user }}" style="margin-right:10px">
                        <i class="far fa-user pr-2" aria-hidden="true"></i><span style="font-size:12px;color:rgba(250,250,250,0.9)">{{ request.user.first_name }} {{ request.user.last_name }}</span>
                    </a>
                    <div class="dropdown-menu dropdown-menu-lg dropdown-menu-left" id="jazzy-usermenu">
                        {% if request.user.admin %}
                            <a href="{% url 'admin_change' pk=request.user.admin.id%}" class="dropdown-item dropdown-footer"><i class="fas fa-user-circle mr-2"></i>{% trans 'Profile' %}</a>
                        {% endif %}
                        <div class="dropdown-divider"></div>
                        <a href="{% jazzy_admin_url request.user request.current_app|default:'admin' %}" class="dropdown-item dropdown-footer"><i class="fas fa-cog mr-2"></i>{% trans 'Account Settings' %}</a>
                        <div class="dropdown-divider"></div>
        ...
        {% if jazzmin_settings.show_sidebar %}
            ...
            <aside class="main-sidebar elevation-4 {{ jazzmin_ui.sidebar_classes }}" id="jazzy-sidebar">
                <a href="{% url 'admin:index' %}" class="brand-link {{ jazzmin_ui.brand_classes }}" id="jazzy-logo">
                    ...
                    <span class="brand-text font-weight-light"><img src="{% static '/logo/myResidence.png' %}" width="70%" style="margin-left:-8px;margin-top: -5px"></span>
                </a>

                <div class="sidebar">
                    <div class="user-panel mt-3 pb-3 mb-3 d-flex">
                        <div class="image">
                                <img src="{{ request.user.admin.image.url }}" width="160px" class="img-circle elevation-2" alt="User Image">
                        </div>
                        <div class="info">
                            ...
                            {% else %}
                                <span class="d-block" style="color: white;">{{ request.user.first_name }} {{ request.user.last_name }}</span>
                            {% endif %}
                        </div>
                    </div>
                    ...
                    <nav class="mt-2">
                        <ul class="nav nav-pills nav-sidebar flex-column {{ jazzmin_ui.sidebar_list_classes }}" data-widget="treeview" role="menu" data-collapsible="false">
                            ...
                            {% if jazzmin_settings.navigation_expanded %}
                                ...
                                <li class="nav-header">Tenant Services</li>
                                {% for app in side_menu_list %}
                                    {% for model in app.models %}
                                        {% if model.name == "Reports" or model.name == 'Repairs' or model.name == 'Visitors' %}
                                                <li class="nav-item">
                                                    {% if model.url %}
                                                    <a href="{{ model.url }}" class="nav-link">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#e080a5"></i> <p>{{ model.name }}</p>
                                                    </a>
                                                {% else %}
                                ...
                                <li class="nav-header">Tenant Billing</li>
                                {% for app in side_menu_list %}
                                    {% for model in app.models %}
                                        {% if model.name == "Billings" or model.name == 'Billing types' or model.name == 'Proof of payments'%}
                                                <li class="nav-item">
                                                    {% if model.url %}
                                                    <a href="{{ model.url }}" class="nav-link">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#e0d380"></i> <p>{{ model.name }}</p>
                                                    </a>
                                                {% else %}
                                                    <span class="nav-link disabled">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#e0d380"></i> <p>{{ model.name }}</p>
                                ...
                                {% for app in side_menu_list %}
                                    {% for model in app.models %}
                                        {% if model.name == 'Tenants' or model.name == 'Tenant registrations' or model.name == 'Log tenants' or model.name == 'Tenant units'%}
                                                <li class="nav-item">
                                                    {% if model.url %}
                                                    <a href="{{ model.url }}" class="nav-link">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#80e082"></i> <p>{{ model.name }}</p>
                                                    </a>
                                                {% else %}
                                                    <span class="nav-link disabled">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#80e082"></i> <p>{{ model.name }}</p>
                            ...
                            <li class="nav-header">Tenant Attrition</li>
                                {% for app in side_menu_list %}
                                    {% for model in app.models %}
                                        {% if model.name == 'Attrition predictions' or model.name == "Tenant contracts" or model.name == "Tenant leases" %}
                                                <li class="nav-item">
                                                    {% if model.url %}
                                                    <a href="{{ model.url }}" class="nav-link">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#9b80e0"></i> <p>{{ model.name }}</p>
                                                    </a>
                                                {% else %}
                                                    <span class="nav-link disabled">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#9b80e0"></i> <p>{{ model.name }}</p>
                            ...
                            <li class="nav-header">Account Management</li>
                                {% for app in side_menu_list %}
                                    {% for model in app.models %}
                                        {% if model.name == 'Admins' or model.name == "Users" or model.name == 'Groups' %}
                                                <li class="nav-item">
                                                    {% if model.url %}
                                                    <a href="{{ model.url }}" class="nav-link">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#80e0dd"></i> <p>{{ model.name }}</p>
                                                    </a>
                                                {% else %}
                                                    <span class="nav-link disabled">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#80e0dd"></i> <p>{{ model.name }}</p>
                            ...
                            <li class="nav-header">Content Management</li>
                                {% for app in side_menu_list %}
                                    {% for model in app.models %}
                                        {% if model.name == "Terms and conditions" or model.name == 'Tenant announcements' or model.name == 'Announcement news'%}
                                                <li class="nav-item">
                                                    {% if model.url %}
                                                    <a href="{{ model.url }}" class="nav-link">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#8090e0"></i> <p>{{ model.name }}</p>
                                                    </a>
                                                {% else %}
                                                    <span class="nav-link disabled">
                                                        <i class="nav-icon {{ model.icon }}" style="color:#8090e0"></i> <p>{{ model.name }}</p>
                ...
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-auto col-sm-3" >
                            <h1 class="m-2" >{% block content_title %}{% endblock %}</h1>
                        </div>
